defmodule ToPdfWeb.JobLive.FormComponent do
  ...
  def update(%{job: job} = assigns, socket) do
    changeset = Consumers.change_job(job)
  ...
  def handle_event("validate", %{"job" => job_params}, socket) do
    changeset =
      socket.assigns.job
      |> Consumers.change_job(job_params)
  ...
  defp save_job(socket, :edit, job_params) do
    case Consumers.update_job(socket.assigns.job, job_params) do
      {:ok, _job} ->
        {:noreply,
         socket
  ...
  defp save_job(socket, :new, job_params) do
    case Consumers.create_job(job_params) do
      {:ok, _job} ->
        {:noreply,
         socket
